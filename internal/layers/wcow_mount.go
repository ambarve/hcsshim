...
func mountProcessIsolatedCimLayersCommon(ctx context.Context, containerID string, volume string, s *scratchLayerData) (_ *MountedWCOWLayers, _ resources.ResourceCloser, err error) {
	log.G(ctx).WithField("containerID", containerID).Info("mount process isolated CIM layers common")
	...
	log.G(ctx).WithField("layer data", layerData).Info("unionFS filter attached")
...
func mountProcessIsolatedBlockCIMLayers(ctx context.Context, containerID string, l *wcowBlockCIMLayers) (_ *MountedWCOWLayers, _ resources.ResourceCloser, err error) {
	...
	log.G(ctx).WithFields(logrus.Fields{
		...
		"scratch layer": l.scratchLayerPath,
		"merged cim":    l.mergedLayer,
		"parent layers": l.parentLayers,
	}).Debug("mounting block CIM layers for process isolated")
	...
	if err != nil {
		return nil, nil, fmt.Errorf("failed mount CIM layers common: %s", err)
	}
	rcl.Add(layerCloser)
	log.G(ctx).WithField("scratch", l.scratchLayerPath).Debug("mounted scratch for process isolated container")
