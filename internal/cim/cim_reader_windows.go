...
func Open(p string) (_ *Reader, err error) {
	...
	mylogger.LogFmt("filesystem header %+v\n", fsh)
...
func (cr *Reader) objReader(o format.RegionOffset, off uint64, size int64) (*io.SectionReader, error) {
...
func (cr *Reader) readOffsetFull(b []byte, o format.RegionOffset, off uint64) (int, error) {
...
func (cr *Reader) readBin(v interface{}, o format.RegionOffset, off uint64) error {
...
func (cr *Reader) walkPath(ino *inode, p string) (*inode, int, error) {
	walked := 0
	for walked < len(p) {
		mylogger.LogFmt("walkPath: walked: %d, len(p): %d, isDir:%v\n", walked, len(p), ino.IsDir())
		if !ino.IsDir() {
			break
		}
		for n := 0; len(p) > walked+n && p[walked+n] == '/'; n++ {
		}
		mylogger.LogFmt("walked:%d, name:%s\n", walked, p[walked:])
		...
		mylogger.LogFmt("walkpath n:%d, name:%s, next:%d\n", n, name, next)
		if name != "" {
...
func (cr *Reader) openAt(dirf *File, p string) (_ *File, err error) {
	fullp := p
	var ino *inode
	mylogger.LogFmt("openAt %+v, path %v\n", dirf, p)
	if len(p) > 0 && p[0] == '/' {
		ino = cr.root
	} else {
		ino = dirf.ino
	...
	mylogger.LogFmt("walked: %d, inode: %+v\n", walked, ino)
	if walked < len(p) {
		if ino.IsDir() {
...
func (cr *Reader) readFile(id format.FileID, file *format.File) error {
	...
	mylogger.LogFmt("fileID %v data dump: %v\n", id, hex.Dump(b))
...
func (cr *Reader) getPESegment(sr *streamReader, off uint64) (int64, uint64, error) {
	if !sr.peinit {
