...
func (uvm *UtilityVM) MountInUVM(ctx context.Context, uvmCimPath string) (_ string, err error) {
	...
	if _, ok := uvm.cimMounts[uvmCimPath]; !ok {
		...
		guestReq := guestrequest.ModificationRequest{
			ResourceType: guestresource.ResourceTypeCimMount,
...
// Returns the path ("\\?\Volume{GUID}\" format) at which the cim at `uvmCimPath` is mounted inside the uvm.
// Throws an error if the given cim is not mounted.
func (uvm *UtilityVM) GetCimUvmMountPath(uvmCimPath string) (string, error) {
	uvm.cimMountMapLock.Lock()
	defer uvm.cimMountMapLock.Unlock()
	ci, ok := uvm.cimMounts[uvmCimPath]
	if !ok {
